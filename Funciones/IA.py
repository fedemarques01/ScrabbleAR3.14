...
def CPUdesarmar(ind,letras): 
    """recibe las letras del atril y las pocisiones ordenadas para formar la palabra"""
    l=[]
    for i in ind:
        l.append(letras[i])
    return l
...
def CPUformarPalabra(letras): 
    """junta la lista de caracteres en 1 solo string para luego comprobarla"""
    s=''
...
def CPUigual(*num): 
    """verifica si no hay numeros repetidos en la lista de indices"""
    num = list(num)
...
def CPUarmar(l,*ind): 
    """arma la lista de caracteres utilizando los indices ordenados y la lista de letras a la cual los indices se refieren"""
    s=[]
...
def CPUahhh(l,dif,word='',*ind): 
    """
    funcion principal de la IA. esta funcion recibe la lista de letras del atril, los tipos de palabras que admite el programa (VB,NN o AJ), 
    una verificacion de recursividad que por defecto es '' y una lista de indices que en un comienzo estan vacios
...
def CPUmain(letras,dif): 
    """main de la funcion que recibe el atril y el tipo de palabra valida (VB o NN por ej)"""
